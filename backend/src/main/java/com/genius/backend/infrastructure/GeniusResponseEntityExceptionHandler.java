...
public class GeniusResponseEntityExceptionHandler extends ResponseEntityExceptionHandler {
	...
	public ResponseEntity<ApiError> handleConstraintViolation(ConstraintViolationException ex, WebRequest request) {
		...
		ApiError apiError = new ApiError(HttpStatus.BAD_REQUEST, new Date(), ex.getClass().getSimpleName(), errors);
	...
	protected ResponseEntity<Object> handleMethodArgumentNotValid(MethodArgumentNotValidException ex, HttpHeaders headers, HttpStatus status, WebRequest request) {
		...
		ApiError apiError = new ApiError(HttpStatus.BAD_REQUEST, new Date(), ex.getClass().getName(), errors);
		return new ResponseEntity<>(apiError, new HttpHeaders(), apiError.getStatus());
	}
	...
	public ResponseEntity<ApiError> handleMethodArgumentTypeMismatchException(MethodArgumentTypeMismatchException ex, WebRequest request) {
		...
		return new ResponseEntity<>(new ApiError(HttpStatus.BAD_REQUEST, new Date(), message), new HttpHeaders(), HttpStatus.BAD_REQUEST);
	}
	...
	public ResponseEntity<ApiError> handleInsufficientAuthenticationException(InsufficientAuthenticationException ex, WebRequest request) {
		return new ResponseEntity<>(new ApiError(HttpStatus.BAD_REQUEST, new Date(), ex.getClass().getName(), ex.getMessage()), new HttpHeaders(), HttpStatus.FORBIDDEN);
	}
